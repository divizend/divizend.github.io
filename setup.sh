...
# S2 Configuration
read -s -p "Enter S2 Access Token: " S2_TOKEN < /dev/tty
echo ""
if [[ -z "$S2_TOKEN" ]]; then echo -e "${RED}S2 Token is required.${NC}"; exit 1; fi

# Resend API Key
read -s -p "Enter Resend API Key (starts with re_): " RESEND_KEY < /dev/tty
echo ""
...
read -s -p "Paste the Resend Webhook Secret here: " RESEND_WEBHOOK_SECRET < /dev/tty
echo ""
