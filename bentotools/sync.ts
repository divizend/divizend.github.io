...
function parseToolsRoot(toolsRoot: string): ParsedToolsRoot {
  ...
  if (!match) {
    throw new Error(`Invalid TOOLS_ROOT_GITHUB format. Expected: https://github.com/owner/repo[/branch][/path]`);
  }
...
async function fetchIndexTs(parsed: ParsedToolsRoot): Promise<string> {
  ...
  if (!branch) {
    console.log(`📡 Fetching default branch for ${parsed.owner}/${parsed.repo}...`);
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch index.ts from ${rawUrl}: ${response.statusText}`);
  }
...
async function compileStreams(indexTsCode: string): Promise<Record<string, BentoStreamConfig>> {
  // Create a temporary file and import it as a module
  // Use a unique filename to avoid conflicts
  const tempFile = `/tmp/bento-sync-index-${Date.now()}-${Math.random().toString(36).substring(7)}.ts`;
...
async function syncStream(
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(`    ⚠ Stream '${name}' sync returned HTTP ${response.status}`);
      if (body) {
...
async function main() {
  ...
  if (!TOOLS_ROOT_GITHUB) {
    console.error("❌ Error: TOOLS_ROOT_GITHUB environment variable is required");
    process.exit(1);
  }
  ...
  if (!S2_BASIN || !BASE_DOMAIN || !S2_ACCESS_TOKEN || !RESEND_API_KEY) {
    console.warn("⚠ Warning: Some environment variables are missing. Stream configs may contain unsubstituted variables.");
    console.warn(`  Missing: S2_BASIN=${S2_BASIN ? "SET" : "MISSING"} BASE_DOMAIN=${BASE_DOMAIN ? "SET" : "MISSING"} S2_ACCESS_TOKEN=${S2_ACCESS_TOKEN ? "SET" : "MISSING"} RESEND_API_KEY=${RESEND_API_KEY ? "SET" : "MISSING"}`);
  }
  ...
  for (const [name, config] of Object.entries(streams)) {
    substitutedStreams[name] = substituteVariables(config, vars) as BentoStreamConfig;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn("This is expected if Bento is not running or not publicly accessible");
      process.exit(0);
    }
  } catch (error) {
    console.warn(`⚠ Warning: Bento API at ${BENTO_API_URL} is not accessible`);
  ...
  console.log(`✅ Stream sync completed (${successCount}/${Object.keys(substitutedStreams).length} successful)`);
}
